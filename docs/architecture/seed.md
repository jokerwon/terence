...
基于 React@19 + Vite@7，推荐 seed 项目结构如下：
...
├── ui/                 # CLI 注入的业务 UI 资产
│   └── OrderForm/
├── engines/            # 项目级 engine 组合（可选）
├── stores/             # 项目级状态（zustand）
...
- 在页面或页面 hook 中创建 engine
- 通过 ui adapter 间接调用 action

```js
const orderEngine = createOrderEngine({ submitOrder })
```

### 4.2 禁止行为

- 在页面中直接推断业务状态
- 在 zustand 中复制 core 状态
- 在多个地方各自维护 engine 状态
...
- 修改 View 结构
- 替换样式与布局
- 扩展 adapter（保持 core 不变）

### 5.3 禁止行为

- 绕过 adapter 直接调用 engine.actions
- 在 seed 中复制 ui 组件逻辑
